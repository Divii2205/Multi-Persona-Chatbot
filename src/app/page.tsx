            ...
            {Object.values(personas).map((persona) => (
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <span className={`font-semibold ${activePersona === persona.id ? 'text-blue-700' : 'text-slate-700'}`}>
                    {persona.name}
                  </span>
                </div>
              ...
              {messages.length === 0 && (
                ...
                >
                  ...
                  <div className="flex flex-wrap justify-center gap-2 max-w-lg">
                    {currentPersonaData.suggestedQuestions.map((q, idx) => (
                      <button
                        key={idx}
                ...
                >
                  <div className={`flex max-w-[85%] sm:max-w-[75%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
                      msg.role === 'user' ? 'bg-blue-100 ml-3' : 'bg-slate-200 mr-3'
                    }`}>
